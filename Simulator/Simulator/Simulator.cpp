...
GLfloat aspectRatio;
std::chrono::milliseconds logicStepDuration = std::chrono::seconds(10);
World world;
int numFrames = 0;
int fps;
...
void stepLogic() {
	...
	world.stepLogic(logicStepDuration);
	if (updateRequired) {
		glutPostRedisplay();
	}
}
...
{
	world.addVisualObject(new Creature(0.0f, 0.0f, 0.0f, &world));
	world.addVisualObject(new Creature(10.0f, 10.0f, 0.0f, &world));
	for (int i = 0; i < 1000; i++) {
		Plant* ptr = new Plant((rand()%1000)/10.0f-50.0f, (rand()%1000)/10.0f-50.0f, 0.0f, &world);
